...
<script setup lang="ts">
  ...
  /**
   * 是否需要自动轮播：仅在「多页」且非全屏查看时开启。
   * slick 的 autoplay 在 infinite 下每 autoplaySpeed 就会触发一次 slideHandler，
   * 即使只有一页也会成对回调 beforeChange / afterChange，因此单页时直接关闭，避免无意义回调。
   */
  const canAutoplay = computed(() => isPlaying.value && paginatedCameras.value.length > 1);
  ...
  const handleBeforeChange = (from: number, to: number) => {
    // 页码未实际变化时（如单页轮播回调）直接忽略，避免重复重挂载播放器
    if (from === to) return;
  ...
  // 注意：不在此重挂载播放器。已访问页的 VideoPlayer 常驻（见 visitedPages），视频会持续播放；
  // 重挂载会销毁 HLS 并重新开始心跳计时，导致心跳上报频率远高于 30s 且成对重复。
