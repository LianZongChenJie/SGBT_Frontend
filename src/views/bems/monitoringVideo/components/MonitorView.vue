...
<script setup lang="ts">
  ...
  /** ===== 播放地址预热 =====
   * 服务端在 localPlayUrl 之后才开始建流（从设备拉流需要数秒到数十秒）。
   * 若等到点「确认」才请求，紧接着发出的 m3u8 清单请求会被网关挂住，
   * 表现为「视频加载很慢甚至超时，点一下刷新却秒出」。
   * 因此勾选时就提前取一次地址，确认时直接复用。
   */
  ...
  /** 预热结果缓存时长：仅在「勾选 → 确认」这个很短的窗口内复用，避免复用过期地址 */
  const PLAY_URL_CACHE_TTL = 60 * 1000;

  const playUrlCache = new Map<string, PlayUrlInfo>();
  const playUrlPending = new Map<string, Promise<PlayUrlInfo | null>>();

  /** 取播放地址：同一 indexCode 复用缓存或进行中的请求，避免重复建流 */
  const fetchPlayUrl = (indexCode: string): Promise<PlayUrlInfo | null> => {
    const cached = playUrlCache.get(indexCode);
    if (cached && Date.now() - cached.at < PLAY_URL_CACHE_TTL) {
      return Promise.resolve(cached);
    }
    const pending = playUrlPending.get(indexCode);
    if (pending) return pending;

    const task = (async (): Promise<PlayUrlInfo | null> => {
      try {
        // 接口返回：url 为 m3u8 播放地址，cameraIndexCode 为摄像头唯一编码
        const res = await getCameraLocalPlayUrl({ cameraIndexCode: indexCode });
        // 排查用：某一路不播放时，先比对这里的原始返回
        console.log('[monitoringVideo] localPlayUrl 返回', indexCode, res);
        const info: PlayUrlInfo = {
        ...
        playUrlCache.set(indexCode, info);
        return info;
      } catch (error) {
        console.error('获取摄像头播放地址失败:', indexCode, error);
        return null;
      } finally {
        playUrlPending.delete(indexCode);
      }
    })();

    playUrlPending.set(indexCode, task);
    return task;
  };

  /**
   * 勾选变化即预热播放地址。
   * 只预热会实际上墙的前 N 路（cameraList 按容量截断），避免一次勾很多路时把服务端流全部拉起。
   */
  watch(
    cameraCheckedKeys,
    (keys, oldKeys) => {
      // 取消勾选的清掉缓存，下次重新勾选时重新取流
      (oldKeys || []).forEach((k) => {
        if (isLeafKey(k) && !keys.includes(k)) {
          const v = cameraLeafMap.get(k);
          const code = v ? getVideoCode(v) : '';
          if (code) playUrlCache.delete(code);
        }
      });

      keys
        .filter(isLeafKey)
        .slice(0, gridCapacity.value)
        .forEach((key) => {
          const v = cameraLeafMap.get(key);
          const code = v ? getVideoCode(v) : '';
          if (code) void fetchPlayUrl(code);
        });
    },
  );
  ...
  const handleCameraSelectConfirm = async () => {
    ...
    try {
      // 取不到播放地址（不会上墙、也不会有心跳）的摄像头名称，便于定位“选了 N 路只播了 M 路”
      const noStreamNames: string[] = [];
      // 逐路并行请求：cameraIndexCode 取选中摄像头（videoList 项）的 indexCode
      const list = await Promise.all(
        leafKeys.map(async (key): Promise<CameraItem | null> => {
          const v = cameraLeafMap.get(key);
          if (!v) {
            console.warn('[monitoringVideo] 已选摄像头在树数据中不存在，已跳过:', key);
            return null;
          }
          const indexCode = getVideoCode(v);
          if (!indexCode) {
            console.warn('[monitoringVideo] 摄像头缺少唯一编码，无法获取播放地址:', v?.name);
          }
          // 复用勾选时预热的结果；缓存过期或未预热成功时这里会重新请求
          const playInfo = indexCode ? await fetchPlayUrl(indexCode) : null;
          // 接口未返回播放地址时，回退到树节点自带 url
          const url = playInfo?.url || v?.url || '';
          // 心跳 streamKey 取摄像头唯一编码 cameraIndexCode
          const streamKey = playInfo?.streamKey || indexCode;
          if (!url) noStreamNames.push(v.name || indexCode || String(key));
          return {
            id: streamKey || key,
            cameraName: v.name,
            url,
      ...
      // 播放地址为空的摄像头不会渲染播放器，也就不会有心跳上报，这里显式提示避免“静默少一路”
      if (noStreamNames.length > 0) {
        message.warning(`以下摄像头未取到播放地址，未上墙：${noStreamNames.join('、')}`);
      }
    } finally {
