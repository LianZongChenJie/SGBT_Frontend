...
/**
 * 摄像头播放心跳续期（GET，streamKey 以 query 参数上报，取摄像头唯一编码 cameraIndexCode）
 * 播放期间按固定间隔上报 streamKey，避免服务端提前回收视频流
 * 说明：successMessageMode 置为 none，避免后端成功返回的 message（如 'ok'）被全局拦截器弹成提示；
 *      失败提示由播放器心跳处统一处理（会做去重，避免每 30s × 多路刷屏）
 */
export const sendCameraHeartbeat = (params: { streamKey: string }) =>
  defHttp.get(
    { url: Api.cameraHeartbeat, params },
