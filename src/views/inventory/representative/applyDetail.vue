<template>
  <BasicModal v-bind="$attrs" @register="registerModal" title="详情" defaultFullscreen :showCancelBtn="false" :showOkBtn="false">
    <template #handle>
      <div class="event-handle">
        <a-button v-if="detailData.status == '待审批'" type="primary" size="small" @click="handleDoAction()">审批</a-button>
      </div>
    </template>
...
<script lang="ts" setup>
  ...
  const emit = defineEmits(['approveHandle', 'register']);
  const [registerModal, { closeModal, setModalProps }] = useModalInner(async (record) => {
    id.value = record.id;
    fetchDetail();
  });
  ...
  function handleDoAction() {
    emit('approveHandle', detailData.value);
  }
  watch(
    () => props.changeFlag,
    () => {
