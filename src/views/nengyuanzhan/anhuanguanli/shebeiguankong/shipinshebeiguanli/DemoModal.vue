...
<script lang="ts" setup>
  ...
  import { saveOrUpdateDemo, getDemoById, getDeviceTypeTreeList } from './demo.api';
  ...
  const [registerModal, { setModalProps, closeModal }] = useModalInner(async (data) => {
    ...
    const treeData = await getDeviceTypeTreeList();
    updateSchema({
      field: 'deviceType',
      componentProps: {
        treeData,
    ...
    if (unref(isUpdate) && record.id) {
      //获取详情
      data.record = await getDemoById({ id: record.id });
      //表单赋值
