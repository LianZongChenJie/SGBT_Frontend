...
<script lang="ts" setup>
  ...
  import { getDemoById, getImportUrlFujian, saveOrUpdateDemoPutong, saveOrUpdateDemoLingyong } from './demo.api';
  ...
  async function handleSubmit() {
    try {
      ...
      if (!spareParts.length) {
        createMessage.warning('请先添加备件明细');
        return;
      }
      const hasInvalidQty = spareParts.some((item) => !item.qty || Number(item.qty) <= 0);
      if (hasInvalidQty) {
        createMessage.warning('请输入大于 0 的出库数量');
        return;
      }
      setModalProps({ confirmLoading: true });
      //提交表单
      let arr = spareParts.map((item) => {
        return {
          deviceId: item.id,
          qty: item.qty,
        };
      ...
      //提交表单
      if (obj.sheetType === '1') {
        await saveOrUpdateDemoPutong(obj, isUpdate.value);
      } else {
        await saveOrUpdateDemoLingyong(obj, isUpdate.value);
      }
      //关闭弹窗
      closeModal();
      //刷新列表
