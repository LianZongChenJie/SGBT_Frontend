  ...
  <BasicModal v-bind="$attrs" @register="registerModal" :title="title" @ok="handleSubmit" width="80%">
    <template v-if="hasDetailData">
      ...
      <div class="flex justify-between ml5 mr5">
        <h2>{{ editFormData?.subject }}出库单</h2>
        <a-space>
      ...
      <div id="printContent">
        ...
        <a-tabs v-model:activeKey="activeKey" class="ml5 mr5">
          <a-tab-pane key="1" tab="出库信息">
            <Chukuxinxi v-if="activeKey === '1'" :data="editFormData" style="min-height: 400px"></Chukuxinxi>
          </a-tab-pane>
          <a-tab-pane key="2" tab="出库明细">
            <Chukumingxi v-if="activeKey === '2'" :data="editFormData" style="min-height: 400px"></Chukumingxi>
          </a-tab-pane>
...
<script lang="ts" setup>
  ...
  import Chukuxinxi from './components/Chukuxinxi.vue';
  import Caozuojilu from './components/Caozuojilu.vue';
  import Chukumingxi from './components/Chukumingxi.vue';
  ...
  const [registerModal, { setModalProps, closeModal }] = useModalInner(async (data) => {
    ...
    if (unref(isUpdate)) {
      ...
      const detailRecord = await getDemoById({ id: data.record.id });
      data.record = {
        ...listRecord,
        ...detailRecord,
