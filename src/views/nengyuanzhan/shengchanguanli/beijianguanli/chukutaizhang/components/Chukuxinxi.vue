...
onMounted(() => {
  watch(
    ...
    async (newValue) => {
      ...
      let deptName = record?.deptName || record?.orgName || record?.departName || record?.sysOrgCode_dictText || employeeInfo?.deptName || '';
      const rawApplicantName =
        record?.applicantName && String(record?.applicantName) !== String(record?.applicant) && !isLikelyId(record?.applicantName)
          ? record.applicantName
          : '';
      let applicantDisplayName = rawApplicantName || employeeInfo?.name || '';
      if (!deptName) {
      ...
      const normalizedRecord = {
        ...
        orgName: record?.orgName || deptName,
      };
      await resetFields();
