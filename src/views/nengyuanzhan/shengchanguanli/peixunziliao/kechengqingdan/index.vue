<template>
  <div style="padding: 10px">
    <BasicTable @register="registerTable" :rowSelection="rowSelection">
      <template #tableTitle>
        <a-button v-if="hasPermission('operation:operation_course:add')" preIcon="ant-design:plus-outlined" type="primary" @click="handleAdd"
          >新增课程</a-button
        >
        ...
        <!--        </a-upload>-->
        <a-button
          v-if="hasPermission('operation:operation_course:exportXls')"
          preIcon="ant-design:export-outlined"
          type="primary"
          @click="handleExportXls('课程清单', getExportUrl, exportParams)"
        >
          导出
        </a-button>
        <a-dropdown v-if="checkedKeys.length > 0 && hasPermission('operation:operation_course:deleteBatch')">
          <template #overlay>
            <a-menu>
              <a-menu-item v-if="hasPermission('operation:operation_course:deleteBatch')" key="1" @click="batchHandleDelete">
...
<script lang="ts" setup name="kechengqingdan">
  ...
  function getActions(record: Recordable) {
    return [
      {
        label: '编辑',
        auth: 'operation:operation_course:edit',
      ...
      {
        label: '删除',
        auth: 'operation:operation_course:delete',
        popConfirm: {
          title: '是否确认删除',
