<template>
  <div style="padding: 10px">
    <BasicTable @register="registerTable" :rowSelection="rowSelection">
      <template #tableTitle>
        <a-button
          v-if="hasPermission('operation:operation_base_fault_category:add')"
        ...
        >
        <a-button
          v-if="hasPermission('operation:operation_base_fault_category:startOrStop')"
        ...
        <!--          导出-->
        <!--        </a-button>-->
        <a-dropdown v-if="checkedKeys.length > 0 && hasPermission('operation:operation_base_fault_category:deleteBatch')">
          <template #overlay>
            <a-menu>
              <a-menu-item v-if="hasPermission('operation:operation_base_fault_category:deleteBatch')" key="1" @click="batchHandleDelete">
...
<script lang="ts" setup name="ziliaolu">
  ...
  import { getDemoList, deleteDemo, batchDeleteDemo, saveUpdateStatus, getTreeListGuzhangleibie } from './demo.api';
    ...
    return [
      {
        ...
        auth: 'operation:operation_base_fault_category:edit',
      },
      // {
      //   label: '启用',
      ...
      {
        label: '删除',
        auth: 'operation:operation_base_fault_category:delete',
        popConfirm: {
          title: '是否确认删除',
