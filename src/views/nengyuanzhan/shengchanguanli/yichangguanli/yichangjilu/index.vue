    ...
    <BasicTable :rowSelection="rowSelection" @register="registerTable">
      <template #tableTitle>
        ...
        <a-button
          ...
          type="primary"
          @click="handleExportXls('异常记录', getExportUrl, exportParams)"
        >
          导出
        </a-button>
      </template>
...
<script setup lang="ts" name="gaojingjilu">
  ...
  const statCards = [
    { key: 'total', label: '异常总数', icon: 'ant-design:bell-filled', theme: 'total' },
  ...
  function getSingleSelectedRecord(actionLabel: string) {
    if (checkedKeys.value.length === 0) {
      createMessage.warning(`请先勾选一条异常记录后再${actionLabel}`);
      return null;
    }
    if (checkedKeys.value.length > 1 || selectedRows.value.length > 1) {
      createMessage.warning(`${actionLabel}时只能选择一条异常记录`);
      return null;
    }
  ...
  function handleSelectedEdit() {
    ...
    if (String(record.status) !== '0') {
      createMessage.warning('当前异常状态不支持处理事件');
      return;
    }
    handleEdit(record);
  }
  ...
  function handleSelectedEliminate() {
    ...
    if (String(record.status) === '2') {
      createMessage.warning('当前异常已消除，不能重复消除');
      return;
    }
    handleDel(record);
  }
