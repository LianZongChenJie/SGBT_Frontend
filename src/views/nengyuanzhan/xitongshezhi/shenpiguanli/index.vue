    ...
    <a-row :gutter="10" style="width: 100%">
      <a-col :span="8" v-for="(list, index) in lists" :key="index" class="mb2">
        <a-card :title="list.formName">
          <template #extra>
          <a-switch
              v-if="hasPermission('operation:operation_approval_config:changeStatus')"
          ...
          <p>审批人：{{ list.approvalNames }}</p>
          <p>操作时间：{{ list.createTime }}</p>
          <div class="flex flex-justify-end">
            <a-button v-if="hasPermission('operation:operation_approval_config:edit')" type="link" @click="handleEdit(list)">设置 </a-button>
          </div>
        </a-card>
      </a-col>
    </a-row>
