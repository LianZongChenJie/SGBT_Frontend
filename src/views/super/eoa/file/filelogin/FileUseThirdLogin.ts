...
import { setThirdCaptcha, getCaptcha, thirdLogin } from '/@/api/sys/user';
import { useI18n } from '/@/hooks/web/useI18n';

export function fileUseThirdLogin(emit) {
  ...
  // 根据token执行登录
  async function doThirdLogin(token) {
    if (!unref(thirdLoginState)) {
      thirdLoginState.value = true;
      await thirdLogin({ token, thirdType: unref(thirdType) }).then((res) => {
        if (res && res.userInfo) {
          emit('success', res);
        }
      });
