<template>
  <div :class="['p-2', `online-list-${ID}`]">
    <!-- 查询条件 -->
    <online-query-form ref="onlineQueryFormOuter" :id="ID" @search="queryWithCondition" @loaded="onQueryFormLoaded"></online-query-form>
    ...
    >
      <template #tableTitle>
        ...
        <a-button v-if="buttonSwitch.export" preIcon="ant-design:export-outlined" type="primary" :loading="exportLoading" @click="onExportExcelOverride"> 导出</a-button>
    ...
    <!-- 表单新增、修改弹框 -->
    <OnlineAutoModal :id="ID" @register="registerModal" @success="reload" @formConfig="handleFormConfig"></OnlineAutoModal>
  ...
  export default {
    ...
    setup() {
      ...
      async function initAutoList() {
        loading.value = true;
        // 1.列配置信息
        let columnResult = await getColumnList(NORMAL);
