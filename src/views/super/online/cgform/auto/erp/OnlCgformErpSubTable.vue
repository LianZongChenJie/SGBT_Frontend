<template>
  <!-- 查询条件 -->
  <online-query-form v-if="mainTableSelectedRowRcord" ref="onlineQueryFormOuter" :id="ID" @search="queryWithCondition" ></online-query-form>
  ...
  >
    <template #tableTitle>
      <a-button v-if="btnShow && buttonSwitch.add" preIcon="ant-design:plus-outlined" type="primary" @click="handleAddRcord">新增</a-button>
      <a-button v-if="btnShow && buttonSwitch.import" preIcon="ant-design:import-outlined" type="primary" @click="onImportExcel">导入</a-button>
      <a-button
        v-if="btnShow && buttonSwitch.export"
      ...
      <a-button v-show="selectedKeys.length > 0" v-if="buttonSwitch.batch_delete" preIcon="ant-design:delete-outlined" @click="handleBatchDelete">
        批量删除</a-button
      >
    </template>
  ...
  <!-- 表单新增、修改弹框 -->
  <OnlineAutoModal :source="ERPSUBTABLE" :id="ID" @register="registerModal" @success="reload" @formConfig="handleFormConfig"></OnlineAutoModal>
