  ...
  >
    <template #fieldValidType="props">
      <a-row type="flex" :class="['row-valid-type', { full: !isCustomRegexp(props.value) }]">
        <a-col :class="['left']">
          <a-select :value="props.value" :options="validTypeOptions" placeholder="空" style="width: 100%" @change="props.triggerChange">
  ...
  export default defineComponent({
    ...
    setup() {
      ...
      function validateDictFieldOrDictText({ cellValue, row }, callback) {
        const { dbFieldName } = row;
        const data = tables!.pageTable!.value!.tableRef!.getTableData();
        const findItem = data?.find((item) => item.dbFieldName === dbFieldName);
        if (findItem?.fieldShowType === 'popup_dict') {
