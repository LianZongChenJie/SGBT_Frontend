...
export function useSysMessage(setLocaleText) {
  ...
  function getHrefText(item) {
    if (item.busType === 'bpm' || item.busType === 'event' || item.busType === 'bpm_task' || item.busType === 'tenant_invite') {
      return '去处理'
    } else {
      return '查看详情'
    }
...
export function useMessageHref(emit, props) {
  ...
  async function goPage(record, openModalFun?) {
    ...
    } else if (record.busType == 'event') {
      router.push({name: "event-event", query: {a: 'tenantSetting'}})
    } else {
   ...
   * @param record
   */
  function isDesignFormComment(record) {
