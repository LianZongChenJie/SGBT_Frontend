<template>
  <BasicModal v-bind="$attrs" @register="registerModal" title="工单授权" @ok="handleSubmit" width="1200px" :minHeight="400">
    <!--工单授权部分-->
    ...
    <template #insertFooter>
      <a-dropdown placement="top">
        ...
        <a-button style="float: left"> 批量操作 <Icon icon="ant-design:up-outlined" /> </a-button>
      </a-dropdown>
    </template>
  </BasicModal>
</template>
<script lang="ts" setup>
  ...
  const [registerModal, { setModalProps, closeModal }] = useModalInner(async (data) => {
    ...
    // update-begin--author:liaozhiyang---date:20240701---for：【TV360X-1684】切换角色工单权限，状态没有清空
    newDesignId.value = '';
    designNameValue.value = [];
    oldDesignId.value = '';
    // update-end--author:liaozhiyang---date:20240701---for：【TV360X-1684】切换角色工单权限，状态没有清空
    //初始化数据
    initData();
  });
