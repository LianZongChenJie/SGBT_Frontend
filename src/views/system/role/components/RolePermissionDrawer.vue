...
<script lang="ts" setup>
  ...
  const [registerDrawer, { setDrawerProps, closeDrawer }] = useDrawerInner(async (data) => {
    ...
    const leafKeys = filterLeafPermissionKeys(unref(treeData) as any[], permResult);
  ...
  /**
   * 获取选中节点及其父节点key（用于提交完整层级）
   */
  function getCheckedWithParentKeys(tree: any[], checked: (string | number)[]) {
    const checkedSet = new Set(checked);
    const result = new Set<string | number>(checked);
    const walk = (nodes: any[], parentPath: (string | number)[] = []) => {
      nodes?.forEach((node) => {
        const currentPath = [...parentPath, node.key];
        if (checkedSet.has(node.key)) {
          parentPath.forEach((key) => result.add(key));
        }
  ...
  /**
   * 仅保留叶子节点权限，避免父节点在层级关联模式下出现整树联动选中
   */
  function filterLeafPermissionKeys(tree: any[], permissionKeys: (string | number)[]) {
    const leafKeySet = new Set<string | number>();
    const walk = (nodes: any[]) => {
      nodes?.forEach((node) => {
        if (node.children?.length) {
          walk(node.children);
        } else {
          leafKeySet.add(node.key);
        }
      });
    };
    walk(tree);
    return permissionKeys.filter((key) => leafKeySet.has(key));
  }
  ...
  async function handleSubmit(exit) {
    const checkedKeyList = unref((getTree() as any).getCheckedKeys()) as (string | number)[];
    const permissionIds = getCheckedWithParentKeys(unref(treeData) as any[], checkedKeyList).join(',');
    let params = {
      roleId: unref(roleId),
      permissionIds,
    ...
    if (exit) {
      ...
      // 没有关闭需要重新获取选中数据
      const permResult = await queryRolePermission({ roleId: unref(roleId) });
      defaultCheckedKeys.value = filterLeafPermissionKeys(unref(treeData) as any[], permResult);
    }
  }
