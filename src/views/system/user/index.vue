<template>
  <div>
    ...
    <UserRole @register="registerRoleModal" @success="reload" :destroyOnClose="true" />
    <!--空间绑定-->
...
<script lang="ts" name="system-user" setup>
  ...
  import UserRole from '@/views/system/user/userRole.vue';
  ...
  function getDropDownAction(record): ActionItem[] {
    return [
      ...
      {
        label: '事件角色',
        onClick: handleEventRole.bind(null, record.id),
      },
      {
        label: '绑定空间',
        onClick: handleSpace.bind(null, record),
      },
    ];
  }
  function handleSpace(record) {
    openSpaceModal(true, { record });
  }
  function handleEventRole(id) {
    openRoleModal(true, { id });
  }
  /**
   * 离职
   * @param userName
